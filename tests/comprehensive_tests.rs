...
fn test_mixed_files_and_directories() {
    ...
    let output = run_rkvr_command(
        &["rmrf", single_file.to_str().unwrap(), project_dir.to_str().unwrap()],
        temp_path,
    );
    ...
    // Verify both removed
    assert!(
        !single_file.exists() && !project_dir.exists(),
...
fn test_edge_case_deeply_nested_paths() {
    ...
    assert!(
        metadata.contains(&format!("cwd: {}", deep_dir.display())),
...
fn test_edge_case_files_with_special_characters() {
    ...
    let metadata = read_metadata(&archive_dirs[0]);
    assert!(
        metadata.contains("file with spaces.txt")
...
fn test_concurrent_operations() {
    ...
    assert_eq!(
        archive_dirs.len(),
        1,
        "Should have one archive directory for concurrent operations"
    );

    let metadata = read_metadata(&archive_dirs[0]);
    for i in 0..5 {
        assert!(
            metadata.contains(&format!("- file_{}.txt", i)),
            "Metadata should contain file_{}.txt",
            i
        );
    }
}

...
fn test_multiple_files_same_directory() {
    ...
    assert!(metadata.contains("- app.log") && 
            metadata.contains("- error.log") && 
            metadata.contains("- debug.log"), 
...
fn test_multiple_files_different_directories() {
    ...
    let archive_dirs = get_archive_dirs(&rmrf_dir);
    assert_eq!(archive_dirs.len(), 3, "Should have three archive directories for three different source dirs");
    
    // Verify each archive has correct metadata
    for archive_dir in &archive_dirs {
        let metadata = read_metadata(archive_dir);
        // Each should have its own correct CWD
        assert!(metadata.contains(&format!("cwd: {}", dir1.display())) ||
...
fn test_directory_archiving() {
    ...
    let metadata = read_metadata(&archive_dirs[0]);
    assert!(metadata.contains(&format!("cwd: {}", temp_path.display())), 
            "Metadata should contain parent directory as CWD. Content:\n{}", metadata);
    assert!(metadata.contains("- project"), 
...
fn test_recovery_functionality() {
    ...
    assert_eq!(recovered_content, "recover this content", "Recovered content should match original");
    
    // Verify archive directory was cleaned up
    let remaining_dirs = get_archive_dirs(&rmrf_dir);
    assert_eq!(remaining_dirs.len(), 0, "Archive directory should be removed after recovery");
} 